...
///</summary>
typedef struct tagGlTextureData
{			
	void *textureData;
	int width;
	int height;
...
typedef struct tagGlLightObj {
	u32 val[16];
} GlLightObj;


///<summary>
/// IGraphics implementation for OpenGL
///</summary>	
...
private:
	...
	void initTextureObject(GlTextureData &textureData);

	// Assigns a GLTexObj to a texture map for rendering ...			
	void setupTextureObject(int textureID);

	// Assigns a GLTexObj to a texture map for tesselating pixel z values ...						
	...
	///<summary>
	/// Sets up the FIFO (does nothing if not needed)
	///</summary>
	void setupFIFO();
	...
	typedef std::map<int, GlTextureData> GxTexMap;
	...
	///</summary>
	GlTextureData myReflectionTexture;
	TextureHandle myReflectionTexHandle;
	GlTextureData myLightMapTexture;
	...
	///</summary>
	GlLightObj mySpecLight;
	GlLightObj mySpotLight;
	float mySpecularTheta;
	float mySpecularPhi;
	Vector myLightPosition;
	Vector myLightDirection;
	GlColor myAmbientColor0;
	GlColor myMaterialColor0;
	GlColor myAmbientColor1;
	GlColor myMaterialColor1;
